...
fn setup_tun_interface() -> Result<(), Box<dyn Error>> {
    // Delete existing tun0 if it exists
        ...
        match stream.read(&mut buffer) {
            ...
            Ok(n) => {
                ...
                for (id, client_stream) in clients_guard.iter_mut() {
                    if *id != client_id {
                        if let Err(e) = client_stream.write(data) {
                            error!("Error writing to client {}: {}", id, e);
                        }
                    }
                }
            }
...
fn server_mode() {
    ...
    // WARNING:: tun only works for linux
            ...
            Ok(n) => {
                match encrypt(&buffer[..n]) {
                    ...
                    Ok(encrypted_data) => {
                        ...
                        if let Err(e) = client.write_all(&serialized_data) {
